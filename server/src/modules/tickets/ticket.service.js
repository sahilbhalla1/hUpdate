...
if (orderTypeCode === 'ZSV1') {
  consultingOrigin = 'SR';
} else if (orderTypeCode === 'ZWO3') {
  consultingOrigin = 'CO';
}
    ...
    if (
      ...
      const isReupload = payload?.isReupload === true || payload?.isReupload === "true";
      ...
      VALUES(?,?,?,?, 'PENDING','UPDATE')`,
        ...
        if (!order) return;
         ...
         if (order?.SP_ORDER) {
          ...
          WHERE id = ?
          AND order_type_id =1
          AND (sp_order_number IS NULL OR sp_order_number = '')`,
          [order.SP_ORDER.trim(), ticketId]
          );
        }
    ...
    return {
      ...
      ticket: {
        ...
        externalTicketNumber: row.external_ticket_number,
         spOrderNumber: row.sp_order_number, 
...
//     sm.status_description,
//     st.stage_label,
//     os.source_name AS order_source_name,
//     ot.order_type_name,
//     t.created_at
//   FROM tickets t
//   JOIN status_master sm
...
//   ORDER BY 
//     CASE 
//       WHEN sm.status_description = 'Open' THEN 1
//       WHEN sm.status_description IN ('Assigned', 'Acknowledged', 'In Process') THEN 2
//       ELSE 3
//     END,
//     t.created_at DESC
...
exports.getRecentHistoryTickets = async ({
  ...
  try {
    ...
    (
      ...
      LEFT JOIN stage_master st ON st.id = t.current_stage_id
      JOIN order_source_master os ON os.id = t.order_source_id
    ...
    (
      ...
      JOIN order_type_master ot ON ot.id = t.order_type_id
      WHERE ${customerCondition}
      AND ot.order_type_name = 'Service Request'
    ORDER BY 
    CASE 
      WHEN sm.status_description = 'Open' THEN 1
      WHEN sm.status_description IN ('Assigned', 'Acknowledged', 'In Process') THEN 2
    ...
    (
      ...
      JOIN order_type_master ot ON ot.id = t.order_type_id
      WHERE ${customerCondition}
      AND ot.order_type_name = 'Escalation'
  ORDER BY 
    CASE 
      WHEN sm.status_description = 'Open' THEN 1
      WHEN sm.status_description IN ('Assigned', 'Acknowledged', 'In Process') THEN 2
...
async function processAttachments({
  ...
  files,
  userId,
  isReupload = false   // ← add this flag
}) {
  ...
  // ✅ Skip required check on reupload — only validate what's being sent
  if (!isReupload) {
    for (const requiredField of config.required) {
      if (!fileMap[requiredField]) {
        throw new Error(`${requiredField} is mandatory`);
  ...
  for (const fieldName in fileMap) {
    ...
    if (existing.length) {
      if (isReupload) {
        // ✅ Replace the existing attachment and reset validation status to PENDING
        await connection.execute(
          `UPDATE ticket_attachments
           SET file_name=?, file_path=?, file_size=?, mime_type=?,
               validation_status='PENDING', validation_remark=NULL,
               validated_by=NULL, validated_at=NULL,
               uploaded_by=?, updated_at=NOW()
           WHERE id=?`,
          [
            file.originalname,
            file.path,
...
const DUPLICATE_MESSAGES = {
  ZSV1: "Open Service Request already exists for this customer and product",
  ZWO3: "Open Complaint already exists for this customer and product",
};

exports.checkDuplicateTicket = async (
  orderTypeCode,
  customerId,
  customerProductId,
  orderTypeId
  ...
  if (!CLOSED_STATUS_MAP[orderTypeCode]) {
    return { isDuplicate: false };
  }

  const closedStatuses = CLOSED_STATUS_MAP[orderTypeCode];
  ...
  if (rows.length > 0) {
    return {
      isDuplicate: true,
      message:
        DUPLICATE_MESSAGES[orderTypeCode] || "Open ticket already exists",
